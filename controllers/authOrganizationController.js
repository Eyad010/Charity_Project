...
exports.signUp = catchAsync(async (req, res, next) => {
    ...
    const newUser = await Organization.create({
        name, email, password, passwordConfirm, phone, address, role
    });
...
exports.login = catchAsync( async (req,res,next)=> {
    ...
    if(!email || !password || !role ) {
        return next(new AppError( 'Please provide an email, password and role', 400));
    }

    // 2) check if user exist and password correct and role correct
    const user = await Organization.findOne({ email }).select('+password');

    if (!user || !(await user.correctPassword(password, user.password)) || !user.correctRole(role)) {
...
exports.restrictTo = (role) => {
    return  (req, res, next) => {
        if(req.user.role !== role){
            return next(new AppError('You do not have the correct permissions to perform this action on this resource.', 403));
        }
        next();
    };
}